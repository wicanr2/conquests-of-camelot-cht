...
MINGW_IMG="${MINGW_IMG:-sq1-mingw}"   # 環境相同，共用既有 mingw image；Dockerfile.mingw 可自行 build
EXE="$ROOT/build/mingw-tree/scummvm.exe"
STAGE="$ROOT/build/win64-full"
DIST="$REPO_ROOT/dist-all"
OUT="$DIST/CAMELOT-CHT-full-win64.zip"

[ -f "$EXE" ] || { echo "!! 找不到 $EXE（先跑 mingw build：docker run ... sq1-mingw make）"; exit 1; }
...
docker run --rm --name cam-winpkg-strip -v "$STAGE:/s" "$MINGW_IMG" x86_64-w64-mingw32-strip /s/scummvm.exe

echo ">> 收集 mingw runtime DLL（objdump 確認只需 SDL2.dll + libwinpthread-1.dll，其餘皆系統內建）"
docker run --rm --name cam-winpkg-sdl2dll "$MINGW_IMG" cat /usr/x86_64-w64-mingw32/bin/SDL2.dll > "$STAGE/SDL2.dll"
docker run --rm --name cam-winpkg-pthreaddll "$MINGW_IMG" cat /usr/x86_64-w64-mingw32/lib/libwinpthread-1.dll > "$STAGE/libwinpthread-1.dll"
